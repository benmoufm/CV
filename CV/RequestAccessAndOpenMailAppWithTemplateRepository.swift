//
//  RequestAccessAndOpenMailAppWithTemplateRepository.swift
//  CV
//
...
class RequestAccessAndOpenMailAppWithTemplateRepository: MailRepository {
    ...
    func sendMail(_ contact: CNContact, _ completion: ((Result<Void>) -> Void)?) {
        ...
        guard let mailTo = contact.emailAddresses.first?.value else {
            completion?(.error(CustomError.mailAddressError as NSError))
            return
        }
        guard let encodedParameters = "mailto:\(mailTo)?subject=\(mailSubject)&body=\(mailBody)"
            .addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) else {
                completion?(.error(CustomError.mailEncodingError as NSError))
                return
        }
        guard let url = URL(string: encodedParameters) else {
            completion?(.error(CustomError.mailUrlError as NSError))
            return
        }
        if UIApplication.shared.canOpenURL(url) {
            UIApplication.shared.open(url)
        } else {
            completion?(.error(CustomError.mailOpenUrlError as NSError))
        }
    }
